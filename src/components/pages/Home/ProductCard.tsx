...
const ProductCard: React.FC<ProductCardProps> = ({ item }) => {
  ...
  const { stock, images, product_name, description, discount_price, price } =
    item;
  const [modalData, setModalData] = useState<Item | null>(null);

  const normalizedImages = normalizeBackendImages(images);
  const primaryImage =
    resolveBackendAssetUrl(normalizedImages[0]) || "/Images/miraz.jfif";
        ...
        <div className="md:p-4 relative">
          ...
          {stock === "unavailable" && (
            <div className="absolute inset-0 md:inset-4 rounded-lg bg-gray-800 opacity-50 flex items-center justify-center">
              <span className="text-red-600 italic text-xl font-semibold">
                Stock Out
              </span>
            </div>
          )}
              ...
              <Image
                ...
                src={
                  resolveBackendAssetUrl(
                    normalizeBackendImages(modalData?.images)[0],
                  ) || "/Images/miraz.jfif"
                }
                alt="cart-tshirt"
              ...
              <div className="flex">
                <div className="space-y-1 md:space-y-2">
                  ...
                  <h1 className="flex items-center gap-2 md:text-xl font-bold text-nowrap">
                    <FaCheckCircle color="green" className="text-xl" /> Added to
                    cart successfully!
                  </h1>
                </div>
              </div>
